...
class Succ_Rate_DBOps(object):
    ...
    def get_all(self, system_name='GFDRS'):
        with self.conn:
            if self.system_name.upper()=='GFDRS':
                self.conn = sq.connect('GFDRS_SUCC_RATE.db') #new line
                c=self.conn.cursor()
...
class APP_DB_OPS(object):
    def __init__(self, appid, app_count, start_count, success_count, error_count, abort_count, success_rate, date_app, system_name ):
        ...
        if self.system_name.upper()=='GFDRS':
            #self.conn = sq.connect('GFDRS_VOL.db') old line
            with sq.connect('GFDRS_VOL.db') as self.conn:
                #self.conn = sq.connect('GFDRS_SR.db')
                c=self.conn.cursor()
                #c.execute('''CREATE TABLE GFDRS_SUCC_RATE_DAILY_SUMMARY_TABLE (date text, errors real, aborts real, start_finished integer, wk integer, month_year text, succ_rate real, stand_dev real, median_r real, skew_r real, kurt_r real, unb_var real, system_name text  )''')
        ...
        #print(c.fetchall())
            ...
            with sq.connect('ETIS_VOL.db') as self.conn:
...
class DEALER_DB_OPS(object):
    def __init__(self, dealer, app_count, start_count, success_count, error_count, abort_count, success_rate, date_app, system_name ):
        self.dealer=dealer
        self.app_count=app_count
        self.start_count=start_count
        self.success_count=success_count
        ...
        if self.system_name.upper()=='GFDRS':
            ...
            with sq.connect('GFDRS_VOL.db') as self.conn:
                #self.conn = sq.connect('GFDRS_SR.db')
                c=self.conn.cursor()
                #c.execute('''CREATE TABLE GFDRS_DEALER_DAILY_TABLE (date text, errors real, aborts real, start_finished integer, wk integer, month_year text, succ_rate real, stand_dev real, median_r real, skew_r real, kurt_r real, unb_var real, system_name text  )''')
                c.execute('''INSERT INTO GFDRS_DEALER_DAILY_TABLE VALUES (:dealer, :app_count, :start_count, :success_count, :error_count,:abort_count , :success_rate , :date_app, :system_name)''',{'dealer': self.dealer, 'app_count': self.app_count,
                          'start_count': self.start_count, 'success_count': self.success_count, 'error_count': self.error_count ,'abort_count': self.abort_count, 'success_rate': self.success_rate, 'date_app': self.date_app, 'system_name': self.system_name })
                self.conn.commit()
                print('GFDRS_DEALER_DAILY_TABLE updated')
